...
void BinarySearchTree::Node::node_balance()
{
	size_t left_size = 0, right_size = 0;
	if (left)
	{
		left_size = left->height();
	}
	if (right)
	{
		right_size = right->height();
	}
...
void BinarySearchTree::Node::left_rot()   // левый поворот
{
	size_t height_d = 0, height_r = 0;
	Node* b = right; 
	Node* d = nullptr;
	if (right->left)
	{
		height_d = right->left->height();
		d = right->left;
	}

	if (right->right)
	{
		height_r = right->right->height();
	}

	if (height_d <= height_r)
	{
		if (d)
		{
			d->parent = this;
...
void BinarySearchTree::Node::right_rot()  // правый поворот
{
	size_t height_b = 0, height_l = 0;
	Node* a = left;
	Node* b = nullptr;
